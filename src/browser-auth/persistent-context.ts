...
export async function openPersistentContext(profile: string, executable: BrowserExecutable, headless: boolean, timeout: number, args: string[] = []): Promise<BrowserContext> {
  const module = await import('playwright-core');
  return (module.chromium as unknown as BrowserLauncher).launchPersistentContext(profile, { channel: executable.channel, executablePath: executable.executablePath, headless, timeout, args });
}
