...
export function isHealthy(account:Account,maxFailures=3):boolean{return account.enabled&&account.authState!=='auth_required'&&account.authState!=='disabled'&&account.failures<maxFailures&&account.cooldownUntil<=Date.now();}
